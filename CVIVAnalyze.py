...
def ConfigurationParameters(CVFileLocation, IVFileLocation, PlotSavingLocation, d, A, Irradiation, SensorType, ScaleTemp, FlatFittingMethod1, FlatFittingMethod2, CapManualf1, CapManualf1Err, CapManualf2, CapManualf2Err, slopef1min, slopef1max, flatf1min, flatf1max, slopef2min, slopef2max, flatf2min, flatf2max, slopef1min_1C, slopef1max_1C, flatf1min_1C, flatf1max_1C, slopef2min_1C, slopef2max_1C, flatf2min_1C, flatf2max_1C, Vmin, Vmax, CapMode, CapOffset, VdepCoeff, PlotDopingProfile, PlotDoubleDerivative, SavePopUpPlots, LCRAccuracy,OpenMeasError, CurrentAccuracy):
    ...
    for line in linesCV:
        ...
        if line.startswith("BiasVoltage"):
            words = line.split("\t")
            LCV1 = count
            # the assumption here being that there are only two CV freqs
            pattern = "LCR_Cp_freq*"
            freqs = fnmatch.filter(words,pattern)
            print ">>> freqs are", freqs
            if len(freqs) != 2:
                print "Error: unexpected number of CV frequencies (I expect 2). Aborting..."
                os._exit()
            cv_freq1_idx = words.index(freqs[0]) if freqs[0] in words else None
            print ">>> cv_freq1_idx is", cv_freq1_idx
            cv_freq2_idx = words.index(freqs[1]) if freqs[1] in words else None
            print ">>> cv_freq2_idx is", cv_freq2_idx
    ...
    for line in linesIV:
        ...
        if line.startswith("BiasVoltage"):
            words = line.split("\t")
            LIV1 = count
            iv_bias_idx = words.index("Bias Current_Avg")
            iv_guard_idx = words.index("Total Current_Avg") if "Total Current_Avg" in words else None
    ...
    for line in linesCV:
        count +=1
        if line.startswith("$"):
            print ">>> the line", line, "starts with $"
            continue
        ...
        if count > LCV1 and count < LCV2:
             ...
             print ">>> appending", abs(float(dataCV[0])), "to BVolCV"
             BVolCV.append(abs(float(dataCV[0])))
             Capf1paral.append(float(dataCV[cv_freq1_idx]) + CapOffset)
             Capf2paral.append(float(dataCV[cv_freq2_idx]) + CapOffset)
             #if len(dataCV) > 4:
             #    Gf1.append(float(dataCV[3]))
             #    Gf2.append(float(dataCV[4]))
             if cv_temp_idx:
                 TempCV.append(float(dataCV[cv_temp_idx]) if not math.isnan(float(dataCV[cv_temp_idx])) else 0)
             if cv_dewpt_idx:
                 DewTempCV.append(float(dataCV[cv_dewpt_idx]) if not math.isnan(float(dataCV[cv_dewpt_idx])) else 0)
    ...
    for line in linesIV:
        ...
        if count > LIV1 and count < LIV2:
             ...
             if iv_dewpt_idx:
                 DewTempIV.append(dataIV[iv_dewpt_idx] if not math.isnan(float(dataIV[iv_dewpt_idx])) else 0)
    ...
    Eg0 = 1.1 # eV 66 # eV # 1.206 eV in http://indico.cern.ch/event/129737/session/3/contribution/24/material/slides/1.pdf
    ...
    poptICslopef1, puncICslopef1, rcICslopef1, dICslopef1 = general_fit(Linearfunc,
                                                                        ...
                                                                        ICslopepar0f1,
                                                                        ICcapslopef1err)
    print ">>> poptICslopef1 is", poptICslopef1
    print ">>> puncICslopef1 is", puncICslopef1
    ...
    if CapManualf1 < 0:
        if FlatFittingMethod2 == 0:
            ...
            poptICflatf1, puncICflatf1, rcICflatf1, dICflatf1 = general_fit(Horizontalfunc,
                                                                            ...
                                                                            ICflatpar0f1,
                                                                            ICcapflatf1err)
            print ">>> poptICflatf1 is", poptICflatf1
            print ">>> puncICflatf1 is", puncICflatf1
    if CapManualf1 > 0:
    ...
    if FlatFittingMethod1 == 0:
        #print ">>> poplogflatf1[0] is", poptlogflatf1[0]
        #print ">>> poptlogslopef1[0] is", poptlogslopef1[0]
        #print ">>> poptlogflatf1[1] is", poptlogflatf1[1]
    ...
    print ">>> len(BVolCV) is", len(BVolCV)
    print ">>> BVolCV is", BVolCV
    print ">>> Vdepf1Log is", Vdepf1Log
    print ">>> the closest value to that in BVolCV is", min(BVolCV, key=lambda x:abs(x-Vdepf1Log))
    print ">>> the index of that value in BVolCV is", BVolCV.index(min(BVolCV, key=lambda x:abs(x-Vdepf1Log)))
    Vdepf1LogIdx = BVolCV.index(min(BVolCV, key=lambda x:abs(x-Vdepf1Log))) 
    print ">>> Vdepf2Log is", Vdepf2Log
    print ">>> the closest value to that in BVolCV is", min(BVolCV, key=lambda x:abs(x-Vdepf2Log))
    print ">>> the index of that value in BVolCV is", BVolCV.index(min(BVolCV, key=lambda x:abs(x-Vdepf2Log)))
    ...
    print ">>> Vdepf1LogIdx is", Vdepf1LogIdx
    print ">>> Vdepf1LogIdx1 is", Vdepf1LogIdx1
    print ">>> LCur is", LCur
    print ">>> len(LCur) is", len(LCur)
    Vdepf1LogIdx_b = LCur.index(min([LCur[Vdepf1LogIdx],LCur[Vdepf1LogIdx1]]))# b for below
    print ">>> Vdepf2LogIdx is", Vdepf2LogIdx
    print ">>> Vdepf2LogIdx1 is", Vdepf1LogIdx
    ...
    TotalIdepf1IC = GCur[Vdepf1ICIdx_b]+abs((GCur[Vdepf1ICIdx]-GCur[Vdepf1ICIdx1])*(Vdepf1IC-BVolIV[Vdepf1ICIdx_b])/(BVolIV[Vdepf1ICIdx]-BVolIV[Vdepf1ICIdx1]))#LCur[Vdepf1ICIdx]
    ...
    TotalIdepf2IC = GCur[Vdepf2ICIdx_b]+abs((GCur[Vdepf2ICIdx]-GCur[Vdepf2ICIdx1])*(Vdepf2IC-BVolIV[Vdepf2ICIdx_b])/(BVolIV[Vdepf2ICIdx]-BVolIV[Vdepf2ICIdx1]))#LCur[Vdepf1ICIdx]
    ...
    print "Total Idep_f1_1C = ", TotalIdepf1IC
    ...
    print "finished calculating doping profile"

    """"""""" CREATING LISTS TO PLOT FITTING RESULTS """""""""

    print "creating logslopef1x"
    ...
    print "done creating logslopef1"

    print "creating logflatf1"
    ...
    print "done creating logflatf1"

    print "creating loghorizf1"
    ...
    print "done creating loghorizf1"

    print "creating logslopef2"
    ...
    print "done creating logslopef2"

    print "creating logflatf2"
    ...
    print "done creating logflatf2"

    print "creating loghorizf2"
    ...
    print "done creating loghorizf2"
    ...
    print ">>> min(ICvolslopef2 is", min(ICvolslopef2)
    print ">>> BVolCV[Vdepf2ICIdx]+30 is", BVolCV[Vdepf2ICIdx]+30
    ICslopef2y = Linearfunc(ICslopef2x, poptICslopef2[0], poptICslopef2[1])
    #print ">>> ICslopef2y is", ICslopef2y

    ICflatf2x = np.linspace(BVolCV[Vdepf2ICIdx]-30,max(ICvolflatf2),40000)
    if FlatFittingMethod2 == 0:
        print ">>> in flatfittingmethod2 0 branch"
        print ">>> poptICflatf2[0] is", poptICflatf2[0]
        ICflatf2y = Linearfunc(ICflatf2x, poptICflatf2[0], poptICflatf2[1])
    if FlatFittingMethod2 == 1:
        print ">>> in flatfittingmethod2 1 branch"
        print ">>> poptICflatf2[0] is", poptICflatf2[0]
    ...
    print "done creating all those lists"
    ...
    print "done figuring out scaling coeffs/units"
    ...
    if EndCapf2Log > 1.:
        ...
        CapUnit = "F"
        CapUnit = "F"
    print "done figuring out EndCapf2 stuff"
    ...
    if df2Log > 1.:
        ...
        ThickUnit = "m"
        ThickUnit2 = "m"
    print "done figuring out df2 stuff"
    ...
    if A >= 1.:
        ...
        AreaUnit = "m$^2$"
        AreaUnit2 = "m2"
    print "done figuring out area stuff"
    ...
    print "done figuring out horizV stuff"
    ...
    axf1IC.format_coord = format_coord
    ...
    axf1IC2.format_coord = format_coord
    ...
    fig4.canvas.mpl_connect('button_press_event',onclick)
    ...
    fig5 = plt.figure(5)
    fig5.canvas.mpl_connect('button_press_event',onclick)
    ...
    fig6.canvas.mpl_connect('button_press_event',onclick)
